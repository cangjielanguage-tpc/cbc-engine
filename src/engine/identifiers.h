...
struct Identifier {
    ...
    Identifier(Symlevel::Offset<T> offs, IO::FileId fileId) :
        value(offs.value | (fileId.id << 24))
    {}
    ...
    Symlevel::Offset<T> GetOffset() const {
        return value & OFFSET_MASK;
    }

    IO::FileId GetFileId() const {
        return (value & FILEID_MASK) >> 24;
    }
...
template <typename T>
struct std::hash<Engine::Identifier<T>> {
